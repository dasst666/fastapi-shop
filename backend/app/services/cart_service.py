...
class CartService:
    def __init__(self, db: Session):
        self.prduct_repository = ProductRepository(db)
    
    def add_to_cart(self, cart_data: Dict[int, int], item: CartItemCreate) -> Dict[int, int]:
        product = self.prduct_repository.get_by_id(item.product_id)
        if not product:
            raise HTTPException(
    ...
    def get_cart_detail(self, cart_data: Dict[int, int]) -> CartResponse:
        if not cart_data:
            return CartResponse(items=[], total=0.0, items_count=0)
        
        product_ids = list(cart_data.keys())
        products = self.prduct_repository.get_multiple_by_ids(product_ids)
        products_dict = {product_ids: product for product in products}
